...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        db = torch.sum(dout, dim=0, keepdims=True).squeeze()
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        D =  w.shape[0]
        z = x.view(-1,D) @ w + b
        out = torch.maximum(torch.zeros_like(z), z)
        cache = (x, w, b, z)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
    ...
    def backward(dout, cache):
        ...
        dx = dx.view(x.shape)
        dw =  (x.view(N,-1).T @ dz)
        db = torch.sum(dz, dim=0, keepdims=True).squeeze()
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    ######################################################################
    # Replace "pass" statement with your code
    prob = torch.exp(x) / torch.sum(torch.exp(x),dim=1,keepdim=True)
    N = x.shape[0]

    loss =  -(torch.log(prob[range(N), y])).sum()
    loss /= N
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        z = X.view(X.shape[0], -1) @ self.params['W1'] + self.params['b1']
        z [z<0] = 0
        scores = z @ self.params['W2'] + self.params['b2']
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        prob = exp / torch.sum(exp, dim=1, keepdim=True)

        prob = prob.double()
        self.params['W1'] = self.params['W1'].double()
        self.params['W2'] = self.params['W2'].double()
        self.params['b1'] = self.params['b1'].double()
        self.params['b2'] = self.params['b2'].double()

        N = scores.shape[0]
        loss =  -(torch.log(prob[range(N), y])).mean() \
          + self.reg * (torch.pow(self.params['W1'], 2).sum() \
                        + torch.pow(self.params['W2'], 2).sum())


        dif_scores = prob.clone()
        dif_scores[range(N), y] -= 1
        dif_scores /= N

        grads['W2'] = (z.t() @ dif_scores) + 2 * self.reg * self.params['W2']
        grads['b2'] = dif_scores.sum(dim=0)
        dz = (dif_scores @ self.params['W2'].t())
        dz[z <= 0] = 0
        grads['W1'] = (X.view(X.shape[0], -1).t() @ dz) + 2 * self.reg * self.params['W1']
        grads['b1'] = dz.sum(dim=0)
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for L in range(1,len(hidden_dims)):
          self.params[f'W{L+1}'] = weight_scale * torch.randn(hidden_dims[L-1], hidden_dims[L], dtype=dtype, device=device)
          self.params[f'b{L+1}'] = torch.zeros(hidden_dims[L], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        cache = {}
        cache['x0'] = X.view(X.shape[0], -1)
        for l in range(1, self.num_layers):
          z = cache[f'x{l-1}'] @ self.params[f'W{l}'] + self.params[f'b{l}']
          activation = z.clone()
          activation[activation<0] = 0
          if self.use_dropout:
              activation, dropout_cache = Dropout.forward(activation, self.dropout_param)
              cache[f'dropout{l}'] = dropout_cache

          cache[f'z{l}'], cache[f'x{l}'] = z, activation

        scores = cache[f'x{self.num_layers-1}'] @ self.params[f'W{self.num_layers}'] + self.params[f'b{self.num_layers}']
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for l in range(1,self.num_layers+1):
            reg_loss +=  self.reg * torch.pow(self.params[f'W{l}'], 2).sum()
        loss += reg_loss
        ...
        grads[f'W{self.num_layers}'] = cache[f'x{self.num_layers-1}'].t() @ dif_scores + 2 * self.reg * self.params[f'W{self.num_layers}']
        grads[f'b{self.num_layers}'] = dif_scores.sum(dim=0)


        for l in torch.linspace(self.num_layers - 1, 1, self.num_layers - 1,dtype=int):

          da = dif_scores @ self.params[f'W{l+1}'].t()
          da[cache[f'z{l}'] <= 0] = 0

          if self.use_dropout:
              da = Dropout.backward(da, cache[f'dropout{l}'])
          
          grads[f'W{l}'] = cache[f'x{l-1}'].t() @ da + 2* self.reg * self.params[f'W{l}']
          grads[f'b{l}'] = da.sum(dim=0)
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict,update_rule=sgd,optim_config={'learning_rate': 1e-2},
                    lr_decay=0.95, num_epochs=15, batch_size=16,print_every=100,
                    device='cuda')
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
